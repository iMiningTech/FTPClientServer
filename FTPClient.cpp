...
const FTPClient::Status& FTPClient::transfer(const String& localFileName,
...
void FTPClient::handleFTP() {
  if (_server == nullptr) {
    ...
    if (controlConnect()) {
      FTP_DEBUG_MSG("Connection to %s:%u established",
                    control.remoteIP().toString().c_str(),
                    control.remotePort());
    ...
    if (dataConnect() < 0) {
      ...
      FTP_DEBUG_MSG("Data connection to %s:%u established",
                    data.remoteIP().toString().c_str(), data.remotePort());
...
int8_t FTPClient::controlConnect() {
  ...
  FTP_DEBUG_MSG("Connection to %s:%d ... %s", _server->servername.c_str(),
                _server->port,
                control.connected() ? PSTR("OK") : PSTR("failed"));
  if (control.connected())
    return 1;
  return -1;
}

bool FTPClient::waitFor(const int16_t respCode,
  ...
  if (!aTimeout.canExpire()) {
    ...
    while (control.available()) {
      char c = control.read();
      // FTP_DEBUG_MSG("readChar() line='%s' <= %c", _serverStatus.desc.c_str(),
      // c);
      if (c == '\n' || c == '\r') {
        ...
        if (respCode != _serverStatus.code) {
          ftpState = cError;
          FTP_DEBUG_MSG("Waiting for code %u but SMTP server replies: %s",
                        respCode, _serverStatus.desc.c_str());
        } else {
          FTP_DEBUG_MSG("Waiting for code %u success, SMTP server replies: %s",
                        respCode, _serverStatus.desc.c_str());
        }
