 ...
 * Inspired by https://github.com/danbicks and his code posted in
 * https://github.com/esp8266/Arduino/issues/1183#issuecomment-634556135
 *
 ...
 *
 *    When using non-blocking mode, be sure to call update() frequently, e.g. in
 * loop().
 */
...
class FTPClient : public FTPCommon {
 public:
  struct ServerInfo {
    ServerInfo(const String& _l,
               const String& _pw,
               const String& _sn,
               uint16_t _p = 21,
               bool v = false)
        : login(_l), password(_pw), servername(_sn), port(_p), validateCA(v) {}
    ServerInfo() = default;
    String login;
    String password;
    String servername;
  ...
  const Status& transfer(const String& localFileName,
                         const String& remoteFileName,
                         TransferType direction = FTP_GET);
 ...
 protected:
  ...
  int8_t controlConnect();  // connects to ServerInfo, returns -1: no connection
